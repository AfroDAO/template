...
class WC_Bookings_Details_Meta_Box {
								...
								if ( ! empty( $person_types ) && is_array( $person_types ) ) {
									...
									foreach ( $person_types as $person_type ) {
										woocommerce_wp_text_input( array(
											...
											'type'          => 'number',
											'placeholder'   => '0',
											'value'         => isset( $person_counts[ $person_type->get_id() ] ) ? $person_counts[ $person_type->get_id() ] : 0,
											'wrapper_class' => 'booking-person',
										) );
									}
								} elseif ( empty( $person_types ) && ! empty( $person_counts ) && is_array( $person_counts ) ) {
									echo '<br class="clear" />';
									echo '<h4>' . __( 'Person(s)', 'woocommerce-bookings' ) . '</h4>';
									...
									foreach ( $person_counts as $person_id => $person_count ) {
										...
										woocommerce_wp_text_input( array(
											...
											'label'         => $person_type ? $person_type->get_name() : __( 'Deleted Person type', 'woocommerce-bookings' ),
											'type'          => 'number',
											'placeholder'   => '0',
											'value'         => $person_count,
											'wrapper_class' => 'booking-person',
	...
	public function meta_box_save( $post_id, $post ) {
		...
		if ( ! empty( $product ) && ( $person_types = $product->get_person_types() ) ) {
			foreach ( $person_types as $person_type ) {
		...
		} elseif ( ! empty( $person_counts ) ) {
			foreach ( array_keys( $person_counts ) as $person_id ) {
