...
class Subscriptio_Event_Scheduler
{
    ...
    public function process_scheduled_events()
    {
        ...
        self::fix_stuck_processing_transactions();
     ...
     * Fix stuck transactions
     *
     * Checks for transactions which have been in processing state for
     * more than five minutes and changes transaction status to error
     *
     * We may only end up in situation like this when there's a fatal error
     * during execution of a corresponding action
     *
     * @access protected
     * @return void
     */
    public static function fix_stuck_processing_transactions()
    {
        // Load list of stuck transactions
        ...
        // Iterate over stuck transactions
        foreach ($query->posts as $transaction_id) {

            // Load transaction object
            $transaction = new Subscriptio_Transaction($transaction_id);

            // Set result to error
            $transaction->update_result('error');
            $transaction->update_note(__('Execution stopped prematurely.', 'subscriptio'));
        }
    }
