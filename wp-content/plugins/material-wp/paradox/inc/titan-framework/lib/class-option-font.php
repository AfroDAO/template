 ...
 * @since	1.4
 */
class ParadoxTitanFrameworkOptionFont extends ParadoxTitanFrameworkOption {
	...
	function __construct( $settings, $owner ) {
		...
		paradox_add_action_once( 'admin_enqueue_scripts', array( $this, 'loadAdminScripts' ) );
		paradox_add_action_once( 'customize_controls_enqueue_scripts', array( $this, 'loadAdminScripts' ) );
		paradox_add_action_once( 'admin_head', array( __CLASS__, 'createFontScript' ) );
		paradox_add_action_once( 'wp_enqueue_scripts', array( $this, 'enqueueGooglefonts' ) );
	...
	/**
	 * Generates CSS for the font, this is used in ParadoxTitanFrameworkCSS
	 *
	 * @param	string               $css The CSS generated
	 * @param	ParadoxTitanFrameworkOption $option The current option being processed
	...
	public static function createFontScript() {
		...
		function tf_select_font_update_preview( $container, doTrigger ) {
			...
			if ( $container.find('iframe').is(':not([data-visible=false])') ) {
				$container.find('iframe').attr('src', '<?php echo ParadoxTitanFramework::getURL( 'iframe-font-preview.php?', __FILE__ ) ?>' + $.param(params) );
			}
			...
			<select class='tf-font-sel-family'>
				...
				if ( $this->settings['show_google_fonts'] ) {
				    ...
				    <optgroup label="Google WebFonts" class='google'>
						<?php
						$allFonts = paradox_titan_get_googlefonts();
	 ...
	 * @param	ParadoxTitanFrameworkCustomizerSection $section The section where this option will be placed
	 * @param	int                             $priority The order of this control in the section
	 * @return	void
	 * @since	1.4
	 */
	public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
		$wp_customize->add_control( new ParadoxTitanFrameworkOptionFontControl( $wp_customize, $this->getID(), array(
 ...
 */
add_action( 'customize_register', 'registerParadoxTitanFrameworkOptionFontControl', 1 );
 ...
 * @since	1.4
 */
function registerParadoxTitanFrameworkOptionFontControl() {
	class ParadoxTitanFrameworkOptionFontControl extends WP_Customize_Control {
		public $description;
		public $params;

		public function render_content() {
			$this->params['show_preview'] = false;
			ParadoxTitanFrameworkOptionFont::createFontScript();
			...
			$value = array_merge( ParadoxTitanFrameworkOptionFont::$defaultStyling, $value );
			...
			<label <?php echo $visibilityAttrs ?>>
				...
				<select class='tf-font-sel-family'>
				    <optgroup label="Web Safe Fonts" class='safe'>
						<?php
						foreach ( ParadoxTitanFrameworkOptionFont::$webSafeFonts as $family => $label ) {
							printf( "<option value='%s'%s>%s</option>",
								$family,
								selected( $value['font-family'], $family, false ),
								$label
