...
class ParadoxTitanFrameworkOptionAjaxButton extends ParadoxTitanFrameworkOption {
	...
	public function ajaxSecurityChecker() {
		if ( empty( $_POST['nonce'] ) ) {
			wp_send_json_error( __( 'Security check failed, please refresh the page and try again.', PARADOX_TF_I18NDOMAIN ) );
		}
	...
	function __construct( $settings, $owner ) {
		...
		while ( count( $this->settings['error_callback'] ) < count( $this->settings['action'] ) ) {
			$this->settings['error_callback'][] = __( 'Something went wrong', PARADOX_TF_I18NDOMAIN );
		}

		foreach ( $this->settings['label'] as $i => $label ) {
			if ( empty( $label ) ) {
				$this->settings['label'][ $i ] = __( 'Click me', PARADOX_TF_I18NDOMAIN );
			}
		}
		foreach ( $this->settings['wait_label'] as $i => $label ) {
			if ( empty( $label ) ) {
				$this->settings['wait_label'][ $i ] = __( 'Please wait...', PARADOX_TF_I18NDOMAIN );
			}
		}
	...
	public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
		...
		$wp_customize->add_control( new ParadoxTitanFrameworkOptionAjaxButtonControl( $wp_customize, '', array(
 ...
 */
add_action( 'customize_register', 'registerParadoxTitanFrameworkOptionAjaxButtonControl', 1 );
function registerParadoxTitanFrameworkOptionAjaxButtonControl() {
	class ParadoxTitanFrameworkOptionAjaxButtonControl extends WP_Customize_Control {
		public $description;
		public $options;

		public function render_content() {
			ParadoxTitanFrameworkOptionAjaxButton::createAjaxScript();
