...
/**
 * ParadoxTitanFramework class
 *
 * @since 1.0
 */
class ParadoxTitanFramework {

	/**
	 * All ParadoxTitanFramework instances
	 * @var array
	 */
	 ...
	 * @var array of ParadoxTitanFrameworkAdminPage, ParadoxTitanFrameworkMetaBox, & ParadoxTitanFrameworkCustomizer
	 */
	private $mainContainers = array();

	/**
	 * All Google Font options used. This is for enqueuing Google Fonts for the frontend
	 * TODO Move this to the ParadoxTitanFrameworkOptionSelectGooglefont class and let it enqueue from there
	 * @var array ParadoxTitanFrameworkOptionSelectGooglefont
	 */
	 ...
	 * @var array of ParadoxTitanFrameworkOption
	 */
	private $adminOptions;

	/**
	 * The CSS class instance used
	 * @var ParadoxTitanFrameworkCSS
	 */
	public $cssInstance;
	...
	private $defaultSettings = array(
		// 'css' => PARADOX_TF_FRONTEND_ENQUEUE, 	    
        // If 'generate', Titan will try and generate a cacheable
	 ...
	 *
	 * @return ParadoxTitanFramework
	 */
	public static function getInstance( $optionNamespace, $settings = array('css' => 'generate', 'multisite' => false)) {
		...
		$newInstance = new ParadoxTitanFramework( $optionNamespace, $settings);
	...
	/**
	 * Creates a new ParadoxTitanFramework object
	 *
	 * @since 1.0
	 *
	 * @param string $optionNamespace The namespace to get options from.
	 */
	function __construct( $optionNamespace, $settings ) {
		...
		$this->cssInstance = new ParadoxTitanFrameworkCSS( $this );

		add_action( 'admin_enqueue_scripts', array( $this, 'loadAdminScripts' ) );
		add_action( 'paradox_tf_create_option_' . $this->optionNamespace, array( $this, 'rememberAllOptions' ) );
		add_filter( 'paradox_tf_create_option_continue_' . $this->optionNamespace, array( $this, 'removeChildThemeOptions' ), 10, 2 );
	 ...
	 * @since 1.2.1
	 *
	 * @param ParadoxTitanFrameworkOption $option The option that was just created.
	 *
	 * @return void
	 */
		...
		if ( ! empty( $option->settings['id'] ) ) {
			...
			if ( is_admin() && isset( $this->optionsUsed[ $option->settings['id'] ] ) ) {
				self::displayFrameworkError(
					sprintf( __( 'All option IDs per namespace must be unique. The id %s has been used multiple times.', PARADOX_TF_I18NDOMAIN ),
	...
	protected function getInternalAdminOptions() {
        ...
        // We need to select other things if we are in a network
        // Added in 0.0.9
	 ...
	 *
	 * @see ParadoxTitanFrameworkOption->getValue()
	 */
	 ...
	 *
	 * @see ParadoxTitanFrameworkOption->setValue()
	 */
	...
	public function saveInternalAdminPageOptions() {
        ...
        // We need to check if we are in a network site
        // Added 0.0.9
	 ...
	 *
	 * @return ParadoxTitanFrameworkAdminPage The created admin page
	 */
	 ...
	 *
	 * @return ParadoxTitanFrameworkMetaBox The created meta box
	 */
	 ...
	 *
	 * @return ParadoxTitanFrameworkCustomizer The created section
	 */
	 ...
	 *
	 * @return ParadoxTitanFrameworkCustomizer|ParadoxTitanFrameworkAdminPage|ParadoxTitanFrameworkMetaBox The created container
	 */
	public function createContainer( $settings ) {
		if ( empty( $settings['type'] ) ) {
			self::displayFrameworkError( sprintf( __( '%s needs a %s parameter.', PARADOX_TF_I18NDOMAIN ), '<code>' . __FUNCTION__ . '</code>', '<code>type</code>' ) );
			return;
		}

		$type = strtolower( $settings['type'] );
		$class = 'ParadoxTitanFramework' . str_replace( ' ', '', ucfirst( str_replace( '-', ' ', $settings['type'] ) ) );
		$action = str_replace( '-', '_', $type );
		$container = false;

		if ( ! class_exists( $class ) ) {
			self::displayFrameworkError( sprintf( __( 'Container of type %s, does not exist.', PARADOX_TF_I18NDOMAIN ), '<code>' . $settings['type'] . '</code>' ) );
			return;
		}
	...
	/**
	 * Hook to the paradox_tf_create_option_continue filter, to check whether or not to continue
	...
	public static function displayFrameworkError( $message, $errorObject = null ) {
		...
		?>
		<div style='margin: 20px; text-align: center;'><strong><?php echo PARADOX_TF_NAME ?> Error:</strong>
			<?php echo $message ?>
			<?php
