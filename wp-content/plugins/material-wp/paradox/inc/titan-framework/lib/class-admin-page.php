...
class ParadoxTitanFrameworkAdminPage {
	...
	function __construct( $settings, $owner ) {
        ...
        // MULTISITE SUPPORT: We decide in which hook to add, based on the multisite
        // Added in 0.0.9
      
        if ($owner->settings['multisite']) {
	...
	public function addTitanCreditText() {
		echo __( "<em>Options Page Created with <a href='http://titanframework.net?utm_source=admin&utm_medium=admin footer'>Titan Framework</a></em>", PARADOX_TF_I18NDOMAIN );
	}
	...
	private function verifySecurity() {
		...
		if ( $screen->id != $this->panelID && $screen->id != $this->panelID.'-network') {
			return false;
		}
	...
	public function createAdminPage() {
		...
		if ( ! empty( $_GET['message'] ) ) {
			if ( $_GET['message'] == 'saved' ) {
				echo ParadoxTitanFrameworkAdminNotification::formNotification( __( 'Settings saved.', PARADOX_TF_I18NDOMAIN ), esc_html( $_GET['message'] ) );
			} else if ( $_GET['message'] == 'reset' ) {
				echo ParadoxTitanFrameworkAdminNotification::formNotification( __( 'Settings reset to default.', PARADOX_TF_I18NDOMAIN ), esc_html( $_GET['message'] ) );
			}
		}
	...
	public function createTab( $settings ) {
		$obj = new ParadoxTitanFrameworkAdminTab( $settings, $this );
		$this->tabs[] = $obj;
	...
	public function createOption( $settings ) {
		if ( ! apply_filters( 'paradox_tf_create_option_continue_' . $this->getOptionNamespace(), true, $settings ) ) {
			return null;
		}

		$obj = ParadoxTitanFrameworkOption::factory( $settings, $this );
		$this->options[] = $obj;

		do_action( 'paradox_tf_create_option_' . $this->getOptionNamespace(), $obj );
